...
const char * const BITCOIN_CONF_FILENAME = "thehashcoin.conf";
const char * const BITCOIN_PID_FILENAME = "thehashcoind.pid";
...
#else
    const char* pszModule = "thehashcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Thehashcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Thehashcoin";
#else
    // Unix
    return pathRet / ".thehashcoin";
#endif
#endif
}
